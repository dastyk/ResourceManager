...
RawData DarferLoader::LoadResource(SM_GUID guid, std::function<char*(uint32_t dataSize)> allocCallback)
{
	...
	if (find->second.compressionType)
	{
		...
		{
		//	PrintDebugString("Used Lz77 then huff, size %ld\n", uncompressedSize);
		...
		{
		//	PrintDebugString("Used Lz77, size %ld\n", uncompressedSize);
			UncompressLz77(data.data, rawsize, buffer);
		}
		else if (find->second.compressionType == 3)
		{
		//	PrintDebugString("Used huff then Lz77, size %ld\n", uncompressedSize);
		...
		{
		//	PrintDebugString("Used huff, size %ld\n", uncompressedSize);
	...
	{
	//	PrintDebugString("No comp, size %ld\n", data.size);
